...
const compileError = (cause: unknown): PluginTypeScriptCompileError => {
  if (
    typeof cause === "object" &&
    cause !== null &&
    "errors" in cause &&
  ...
  return new PluginTypeScriptCompileError({
    detail: `TypeScript compilation failed: ${detail}`.slice(0, 4_000),
    cause,
  });
};
...
export const compilePluginTypeScriptEntrypoint = Effect.fn(
    ...
    Effect.mapError(
      (cause) =>
        new PluginTypeScriptCompileError({
          detail: "TypeScript compilation failed: could not create the output directory",
          cause,
        }),
    ),
  );
  ...
  const result = yield* Effect.acquireUseRelease(
    Effect.tryPromise({ try: () => context(buildOptions), catch: compileError }),
    (buildContext) =>
      Effect.tryPromise({
        try: (signal) => {
  ...
  for (const sourcePath of Object.keys(result.metafile?.inputs ?? {})) {
    ...
    if (!isContained(path, input.packageDirectory, absoluteSourcePath)) {
      return yield* new PluginTypeScriptCompileError({
        detail: `TypeScript compilation failed: import escapes the plugin package (${sourcePath})`,
      });
    }
  }
