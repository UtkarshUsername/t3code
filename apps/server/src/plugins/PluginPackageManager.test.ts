...
`;
  ...
  it.effect("retries dependency-blocked chains until every enabled package can activate", () =>
    Effect.gen(function* () {
      const fileSystem = yield* FileSystem.FileSystem;
      const baseDir = yield* fileSystem.makeTempDirectoryScoped({
        prefix: "t3code-plugin-package-dependency-chain-test-",
      });
      const packages = [
        {
          id: "com.acme.a-consumer",
          commandId: "acme.chain.consumer",
          requires: ["acme.middle@1"],
          provides: [],
        },
        {
          id: "com.acme.b-middle",
          commandId: "acme.chain.middle",
          requires: ["acme.root@1"],
          provides: ["acme.middle@1"],
        },
        {
          id: "com.acme.c-provider",
          commandId: "acme.chain.provider",
          requires: [],
          provides: ["acme.root@1"],
        },
      ] as const;
      for (const pluginPackage of packages) {
        const directory = `${baseDir}/userdata/plugins/${pluginPackage.id}`;
        yield* fileSystem.makeDirectory(directory, { recursive: true });
        yield* fileSystem.writeFileString(
          `${directory}/t3-plugin.json`,
          encodeManifest({
            ...manifest,
            id: pluginPackage.id,
            requires: [...pluginPackage.requires],
            provides: [...pluginPackage.provides],
            contributes: { commands: [pluginPackage.commandId] },
          }),
        );
        yield* fileSystem.writeFileString(
          `${directory}/index.mjs`,
      ...
      yield* useEnvironment(
        ...
        Effect.gen(function* () {
          const manager = yield* PluginPackageManager.PluginPackageManager;
          const catalog = yield* PluginCommandCatalog.PluginCommandCatalog;
          for (const pluginPackage of packages) yield* manager.enable(pluginPackage.id);
          expect((yield* manager.status).packages).toEqual(
            expect.arrayContaining(
              packages.map((pluginPackage) =>
                expect.objectContaining({ id: pluginPackage.id, state: "active" }),
              ),
            ),
          );
          expect((yield* catalog.list).commands.map(({ id }) => id)).toEqual(
            expect.arrayContaining(packages.map(({ commandId }) => commandId)),
          );
        }),
      );
