...
function getGitStatusTargetKey(target: GitStatusTarget): string | null {
  ...
  return `${target.environmentId}:${target.executionTarget?.kind === "wsl" ? `wsl:${target.executionTarget.distroName}:${target.executionTarget.user ?? ""}` : "local"}:${target.cwd}`;
}
...
export function refreshGitStatus(
  ...
  const refreshPromise = resolvedClient
    .refreshStatus({
      cwd: target.cwd,
      ...(target.executionTarget !== undefined ? { executionTarget: target.executionTarget } : {}),
    })
    .finally(() => {
...
function subscribeToGitStatusTarget(
  ...
  const syncClientSubscription = () => {
    ...
    currentUnsubscribe = subscribeToGitStatus(targetKey, target, resolved.client);
  };
